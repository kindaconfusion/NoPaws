...
class _BusPageState extends State<BusPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        drawer: MainDrawer(),
        appBar: AppBar(title: const Text('JagTran')),
  ...
  FutureBuilder<List<Map<Bus, Route>>> routeListBuilder() {
    return FutureBuilder<List<Map<Bus, Route>>>(
        ...
        builder: (BuildContext context,
            AsyncSnapshot<List<Map<Bus, Route>>> snapshot) {
          if (snapshot.hasData) {
...
class JagTran {
  Future<List<Map<Bus, Route>>> getRoutes() async {
    ...
    routes = (json.decode(routesJson) as List)
        .map((i) => Route.fromJson(i))
        .toList();
    List<Bus> buses =
        (json.decode(busesJson) as List).map((i) => Bus.fromJson(i)).toList();
    ...
    for (var bus in buses) {
      ...
      list.add({bus: (routeNums[bus.routeId] ?? const Route(id: 0, name: ""))});
    }
    return list;
  }
  ...
  Future<Image> getMap(List<Bus> buses) async {
    ...
    if (buses.isEmpty) {
      return Image.network(
          "https://open.mapquestapi.com/staticmap/v5/map?key=$apikey&boundingBox=30.70309,-88.19106000000001,30.690920000000002,-88.18334999999999&margin=75");
    }
    double n = -1000;
    double e = -1000;
    double s = 1000;
    ...
    String locations =
        latLongPairs.toString().substring(0, latLongPairs.length - 2);
    n += 0.005;
    s -= 0.005;
    e -= 0.005;
    w += 0.005;
    ...
    return Image.network(
        "https://open.mapquestapi.com/staticmap/v5/map?key=$apikey&boundingBox=$n,$w,$s,$e&locations=$locations&margin=50");
  }
}
